...
(function () {
    ...
    */
    var caseAction = function () {
        ...
        var hideFirstDeleteBtn = function () {
            // case editor
        ...
        (function () {
            ...
            if ($caseResultPage.length) {
                ...
                if (!cid) {
                    var url = 'http://' + host + '/restapi/soa2/10290/createclientid?systemcode=09&createtype=3&contentType=json';
                    ...
                    Ajax.get(url, function (data) {
                        ...
                        if (cid) {
                            var now = new Date();
                            var threeYearLater = new Date(now);
                            threeYearLater.setFullYear(threeYearLater.getFullYear() + 3);

                            Cookie.set('cid', cid, threeYearLater.toUTCString(), '/');
                            sentReq();
                        } else {
                            UI.showError('创建uid失败');
                        }
                    }, function (error) {
                        UI.hideLoading();
                        UI.showError('创建uid失败');
